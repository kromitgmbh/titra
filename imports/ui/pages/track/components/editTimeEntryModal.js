...
Template.editTimeEntryModal.onRendered(() => {
  ...
  templateInstance.$('#edit-tc-entry-modal').on('show.bs.modal', () => {
    if (templateInstance.data?.tcid?.get()) {
      bodyInstance = Blaze.renderWithData(
        Template.tracktime,
        { tcid: templateInstance.data?.tcid },
        templateInstance.$('#editTimeEntryModalBody')[0],
      )
      ...
      bodyInstance = Blaze.renderWithData(
        Template.tracktime,
        {
          dateArg: templateInstance.data?.selectedDate,
          projectIdArg: templateInstance.data?.selectedProjectId,
        },
        templateInstance.$('#editTimeEntryModalBody')[0],
      )
    }
