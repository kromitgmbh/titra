...
class BaseChart {
	...
	configure() {
		...
		jQuery(window).on('resize', this.boundDrawFn);
		jQuery(window).on('orientationchange', this.boundDrawFn);
	}

	destroy() {
		jQuery(window).off('resize', this.boundDrawFn);
		jQuery(window).off('orientationchange', this.boundDrawFn);
	}
